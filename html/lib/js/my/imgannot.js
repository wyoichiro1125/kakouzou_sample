/// imgannot.js by KANZAKI, Masahide. ver 3.19, 2022-04-06. MIT license.
"use strict";
var at, Mav = {}, Mau, Mwa;
...
var Mia = {
	const: {
		...
		noimage2: "/works/2016/pub/images/noimage2.png",
		nomanifest: "/works/2016/pub/images/nomanifest.png",
		libpath: "./lib/js/my/",
	...
	use_abs_const: function(){
		for(var key in this.const){
			if(typeof(this.const[key]) !== "string") break;
			this.const[key] = Mia.const.use_abs + this.const[key];
		}
	},
	optv: {
	...
	env : {
		...
		center: null,
		loaded_uri: null,
		cors_proxy: "/works/2016/pub/cors_proxy/", //"https://cors-anywhere.herokuapp.com/",
		is_local: null,
	...
	setup: function (url, data, moreurl){
		if(Mia.const.use_abs) Mia.use_abs_const();
	...
	set_job: function(res){
		...
		case "threed":
			if(res.tile.length){
				this.init(res.tile, res.anno, {anno_count: res.anno_count});
			}else{
		...
		case "image":
		default:
			if(res.tile.length) this.init(res.tile, res.anno, {anno_count: res.anno_count});
			else unsupported(res, this);
		}
		if(Mia.env.is_local && Muib.local) Muib.local.job(res);
		
		function unsupported(res, that){
			var msg, thumb = Mia.jsource.thumbnail;
			if(thumb){
				console.warn("so, no tile to process. trying to show its thumbnail.", res, thumb);
				var id = typeof(thumb) === "string" ? thumb: thumb["@id"],
				msg = res.info.error || "No appropiate tile found.",
				desc = {};
				desc[id] = {
					"description":"<span class='hot'>" + msg + 
					"; However, this manifest contains a thumbnail, which is now displayed.</span>",
					"need_refit": true
				};
				var url = id.match(/^http/) ? id : Mut.uri.resolve(id, that.opts.base);
				Miiif.cview.paged = Muib.state.paged_zoom = true;
				that.init([{"type":"image","url":url}],"", {"page":desc});
				Muib.meta.add(Mia.jsource);
				return;
			}else if(res.info.error_reported){
				msg = res.info.error;
				console.error("so, no tile to process.", msg, res);
			}else{
	...
	init: function (tile, oa, extinfo, type){
		...
		if(type === "image"){
			if(this.filter && this.filter.ctrlbtn) this.filter.makesure_partof_dom(this);
			else this.filter = new Filter(this);
		}
		function viewer_msg(an_count){
			var msg = "preparing viewer";
			if(an_count) msg += " (" + an_count + " annotaions)";
			Muib.appb.set_msg(msg + " ...");
		}
	},
	prep_osd: function (tile, type){
		var osdoption = {
			...
			maxZoomPixelRatio: this.opts.v.mz || Mia.const.nvmaxzoom,
		...
		if(this.keyuris.length === 0 || !this.keyuris[0]){
			console.warn("no valid keyuris", tile[0]);
		}else if(!(document.querySelector("head meta[property]")) && Object.keys(this.cinfo).length)
		Muib.appb.set_ogp(Miiif.full2mid(this.cinfo[this.keyuris[this.env.syncpage || 0]].imgurl, 600));
	...
	post_osd: function(){
		...
		else if(ov.clip === "all") Muib.anno.clip.allimg.checked = true;
	...
	setup_extinfo: function(info){
		...
		}else if(info.hasOwnProperty("anno_count")){
			this.anno_count = info.anno_count;
		}else{
	...
	prepare_env: function(msg){
		...
		this.elt.jldb.onclick = function(){Muib.anno.jldpanel.toggle(this);};
	...
	proc_loaded: function(e, newuri){
		...
		Muib.appb.set_msg("on proc_loaded ...");
			...
			if(!dim){
				...
				if(ci.dim.x !== dim.x || ci.dim.y !== dim.y){
					...
					if(rratio > 1.25 || rratio < 0.75){
						if(ci.layer && ci.layer.length===2 && rratio > 0.4 && rratio < 0.6) ;
						else console.warn("different aspect ratio: original", r[0], "source", r[1]);
...
var Mpj = {
	...
	proc_annot: function (def){
		...
		function set_anno(res, procres){
			res.anno = procres;
			res.anno_u = Object.keys(procres);
			res.anno_u.forEach(function(u){res.anno_count += procres[u].length;});
			if(res.anno_u.length > 50){
				Muib.appb.set_msg("preparing annotation (" + res.anno_count + ") ...");
				console.log(res.anno_count + " annotations in " + res.anno_u.length + " uris");
			}
		}
	},
	...
	get_more_annot: function (json, objanno, idx){
		...
		function proc_one(def){
			...
			if(ctxt === that.ctxs.webanno){
				...
				if(def.structures){
					if(Mia.struct.data.length > 0) Mia.struct.refresh();
					Mia.struct.data = def.structures;
				}
	...
	set_label: function(obj, uri){
		...
		if(obj.label){
			...
			if(labl.match(/^\s*$/)) labl = undefined;
			else this.set_maxlabel(labl);
...
Mpj.webannot = {
	...
	proc_target: function(annogrph, annot){
		...
		if(url.frag) this.register_anno(annot, annogrph, url, keyuri);
	...
	register_anno: function(annot, annogrph, url, keyuri){
		...
		if(annogrph.body){
			if(annogrph.body instanceof Array) this.multi_body(annot, annogrph, url, this.info);
			else this.single_body(annot, annogrph, url);
		} else
	...
	select_one_body: function(angrph, bodyp){
		if(!angrph[bodyp]) {
			angrph[bodyp] = {"value": "", "type": "TextualBody"};
			return angrph;
		}
	...
	body_choice: function(bodytype, items){
		...
		if(bodytype === "Choice")
			...
			if(typeof(items[0]) === "string") bodyvalue = {
				...
				var vals = [];
				items.forEach(function(item){vals.push(item.value);});
				bodyvalue = {
					type: "TextualBody",
					value: vals.join("\n")
				};
...
var Manno = {
	...
	setup_annotations: function (){
		...
		anno_count = Mia.anno_count,
		has_anno = 0,
		n = oauris.length;
		if(n) for(var i=0; i<n; i++) {
			has_anno += this.oa2annotorious(annot, Mia.oa, oauris[i], this.frags);
		}
		...
		function show_progress(i, n, anno_count, has_anno){
			var pct = anno_count ? has_anno / anno_count : i / n,
			msg = "Setup annotations " + Math.ceil(pct * 100) + "%";
			if(anno_count > 10000) console.log(msg);
			Muib.appb.set_msg(msg);
		}
	},
	...
	oa2annotorious: function (annot, oa, uri, frags){
		...
		for(var i=0, n=page_oa.length; i<n; i++){
			...
			anid = page_oa[i].id;
			if(this.registry.has(anid)) continue;
			if(anid && edited[anid]){
				...
				if(this.registry.has(aa.id)) continue;
			...
			if(page_oa[i].target.svg){
				...
				if(page_oa[i].svgid) aa.svgnode = Mut.dom.get(page_oa[i].svgid, "id");
				else page_oa[i].annot = aa;
			}
			annot[page].push(aa);
			this.registry.add(aa.id);
			ancount++;
		}
	...
	resolve_deferred: function(idx, target_uris){
		...
		annopage_count = 0;
		annopage_count += resolver("paint", usetp, this.set_osd_overlay);
		["highlight", "link"].forEach(function(mtvn_type){
		...
		function resolver(mtvn_type, use_overlay, overlay_fn, tganno){
			...
			if(!tganno){
				if(use_overlay) tganno = that.tp;
				else tganno = that.page;
			}
	...
	set_osd_overlay: function(uri, oas, mtvn_type){
		...
		oas.forEach(function(oa){
			...
			else if(mtvn_type === "link") elt.onclick = function(e){e.stopPropagation();};
			cinfo.overlay[sig] = {"element": elt, "location": rect};
		});
	...
	flush: function(newuri){
		if(this.page[newuri][0] && this.page[newuri][0].pix){
	...
	multi_annot: function (annot, frags){
		...
		more.forEach(function(i){
			...
			Muib.anno.popup.add_more(this.current[i].text, this.current[i].creator, i);
	...
	counter: {
		update: function(delta, other_cont){
			...
			function anno_count(delta, that, pagecount, other_cont){
				...
				if(pagecount === 0) Muib.anno.abox.btn.disabled = true;
				else if(pagecount ===1 && delta) Muib.anno.abox.btn.disabled = false;
			}
		},
	...
	anno2oa: {
		get_all: function(as_page_chbx){
			...
			if(Miiif.use && (!Mia.opts.v.au || Miiif.cview.paged)){
				var annolist = [];
				if(as_page_chbx.checked){
					this.get_cnvs_annot(Mia.key_uri(), annolist);
				}else for(var cvuri in Manno.page){
...
Manno.svg = {
	...
	set_overlay: function(uri, oas, add_count){
		...
		function circle_path(ata){
			...
			return "M" + (ata[0] - ata[3]) + "," + ata[1]
				+ arc + (ata[0] + ata[3]) + "," + ata[4]
				+ arc + (ata[0] - ata[3]) + "," + ata[4];
		}
	},
	...
	flush_overlay: function(ci){
		if(!this.svg_node ||
			!this.svg_node._svg.parentNode.parentNode.parentNode) this.svg_node = Mia.osdv.viewer.svgOverlay();
...
var Miiif = {
	...
	get_type: function(r, idx=0){
		...
		return typeval ? (
			Mut.obj.get_one(typeval, idx).replace(/^\w+:/, "") 
		) : undefined;
	},
	check_idattr: function(obj, where){
		if(obj[this.a.id]) return this.a.id;
		console.warn(this.a.id + " not found in " + where + ". will use " + this.a.failid + " instead");
		return this.a.failid;
	},
	...
	proc_manifest_items: function(def, tile, info){
		var annot;
		this.check_options(def);
		if(def.thumbnail) this.vinfo.thumbnail = Mut.obj.get_oneuri(def.thumbnail);
		if(def.items){
	...
	find_prop_used: function(object, propname){
		...
		else if(found_pos > 0) console.log(propname, "property set to", used_prop);
		return used_prop;
	},
	...
	proc_one_embed: function(annors, annot, idx, cvinst, context){
		...
		if(bb instanceof Array){
			var bbarr = [], bi = 0, glue = ";", formats = {text:[], html:[]};
			for(var i=0,n=bb.length; i<n; i++){
				...
				if(bbval[0] && purpose !== "tagging"){
					if(!body) body = bb[i];
					bbarr.push(bbval[0]);
					if(bb[i].format === "text/html") formats.html.push(bi);
					else formats.text.push(bi);
					bi++;
				}
			};
			if(!body) body = bb[0];
		...
		function proc_has_fragment(tt, tgparts, annoid){
			...
			if(svgfrag){
				...
				set_annot(Mia.defer.svg[idx], surl, annoid, val, body, frag, motivation, tgparts, tt, svgfrag);
				 ...
				 (((Mia.opts.v.tp || (cvinst && cvinst.duration))
						&& motivation.match(/^(paint|highlight)ing$/))
					|| motivation.match(/^(link)ing$/))
				&& !Miiif.searchs.req
			){
		...
		function get_val_from_body(body, annors, tgid, trgt){
			...
			case "Image":
				...
				break;
			default:
				if(typeof(body.value) === "string") val = format_text(body, tgid, trgt);
				else if(body.label) val = Mut.str.lang_val(body.label);
				else console.warn("unknown type", type, body[that.a.type]);
			}
			return [val, type];
		}
	...
	add_other_content: function (uri, othercon, fidx_offset=0){
			...
			OpenSeadragon.makeAjaxRequest({
				...
				"success": function(xhr) {
					...
					if((uri_b = Mut.scheme.exists_another(oa, uri, true))){
						...
						var stranges = 0;
						for(var u in oa){
							if(Mia.cinfo[u]) merge_annot(annot, oa, u, u, tgindx);
							else{
		...
		function test_complete(keyuri, curidx){
			tested_idx++;
			if((!fidx_offset && (tested_idx === othercon.length))
				|| (fidx_offset && (curidx === finidx))){
	...
	full2mid: function (url, width=1000, range){
		return url + (this.level.defimg ? ("/" + (range || "full") + "/" + width + (typeof(width)==="number" ? "," : "") + "/0/" + this.level.defimg) : "");
	},
	...
	proc_selections: function(def){
		...
		function proc_selcv(manifest, sel){
			...
			if(sel.canvases) sel.canvases.forEach(function(c, i){register_canvas(c, i, tgids, sel.label);});
			if(sel.members) sel.members.forEach(function(c, i){register_canvas(c, i, tgids, sel.label);});
	...
	ssl: {
		status: 0,
		setup: function(manifest_uri){
			this.status = (location.protocol === "http:" || manifest_uri.match(/^http:/)) ? 1 : 2;
		},
		test: function(infojson){
			if(this.status === 1) return infojson;
			else if(this.status === 2){
				if(infojson.match(/^http:/)){
					console.log("info.json from http: while manifest from https:. This viewer will rewrite the scheme part.");
					this.status = 3;
				}else{
					this.status = 1;
					return infojson;
			...
			return infojson.replace(/^http:/, "https:");
		}
	}
};
...
Miiif.collection = {
	...
	proc: function(def){
		...
		this.thumb_mode = (Mia.opts.v.mode && Mia.opts.v.mode.substr(0,3) === "tbl") ? "tbl" : (
			(def.vhint === "use-thumb" || Mia.opts.v.vhint === "use-thumb") ? "flex" : "auto");
		if(this.thumb_mode === "tbl"){
			this.simple_table(def, div, Mia.opts.v.mode.substr(3));
			this.sate.check(false);
		}else if(!def.collections && this.thumb_mode === "flex"){
			this.div_thumb_box(def, div);
			this.sate.check(false);
		}else{
			...
			else if(this.nested === 0) pul.classList.add("mu");
		}
	...
	list_items: function(def, pnode, len){
		...
		pul.className = "collection" + ((len && len > 20 && this.thumb_mode === "auto") ? " mu" : "");
		if(def.collections){
			this.list_collections(def.collections, pul);
			found = true;
		}
		if(def.manifests){
			if(this.thumb_mode === "flex" && (!def.collections && !def[this.member_prop])){
				var li = Mut.dom.elt("li");
				pul.appendChild(li);
				pul.classList.add("thumb");
				this.thumb_box(def, li);
			}else{
	...
	gen_one_li: function (item){
		...
		if(item.thumbnail) this.one_thumb(li, item.thumbnail);
		return li;
	},
	one_thumb: function(li, thumb){
		var thumburi = Mut.obj.get_oneuri(thumb),
		thumbelt = (Mia.opts.v.thumb === "show") ?
		Mut.dom.elt(
			"img", "", [
				["class", "sthumb"],
				["src", thumburi]
			...
			"span", "💬", [
				...
				["onclick", "Miiif.collection.showthumb(this, \"" + thumburi + "\")"]
			]
		);
		Mut.dom.append(li, [" ", thumbelt]); 
	},
	...
	get_ap_link: function (item, idattr){
		...
		is_manifest = (item[Miiif.a.type] === Miiif.v.manifest || item[Miiif.a.type] === Miiif.v.collection);
		if(typeof(id) === "undefined") return [ap, qv, false];
	...
	thumb_box: function(def, pnode){
		...
		def.manifests.forEach(function(man){
			...
			var capt = man.description ? "<strong>" + Mut.str.lang_val(man.label) + "</strong>: " +  Mut.html.fold_text(man.description, "slong") : Mut.str.lang_val(man.label),
			link = this.get_ap_link(man, Miiif.a.id),
			imgsrc = link[2] === false ? Mia.const.nomanifest : (Mut.obj.get_oneuri(man.thumbnail) || 
				(link[0] ? Mia.const.noimage2 : Mia.const.nomanifest)),
			img = "<img src=\"" + imgsrc + "\"/>",
			cls = link[0] ? "" : "class=\"direct \"",
			ancimg = link[2] === false ? img : "<a " + cls + "href=\"" + link.join("") + "\">" + img + "</a>";
			
			fbhtml += "<figure>" + ancimg +
			"<figcaption>" + capt + related +
			"</figcaption></figure>\n";
		}, this);
		fbox.innerHTML = fbhtml;
		pnode.appendChild(fbox);
		if(!this.thumb_init_done){
			document.body.style.paddingRight = "5px";
			document.body.style.maxWidth = "none";
			this.thumb_init_done = true;
		}
	},
	fboxes: [],
	inc: {},
	thumb_per_div: 24,
	thumb_count: 0,
	div_thumb_box: function(def, pnode){
		var pfbox = Mut.dom.elt("div", "", [["class", "pfbox"]]),
		fbhtml = "", related;
		this.thumb_count = def.manifests.length;
		var fbpos = -1, tgattr = this.thumb_count > 100 ? " target=\"iiifmanifest\"" : "";
		def.manifests.forEach(function(man, i){
			if(i % this.thumb_per_div === 0){
				if(fbhtml) this.fboxes[fbpos].innerHTML = fbhtml;
				fbpos++;
				fbhtml = "";
				this.fboxes.push(Mut.dom.elt("div","",[["class", "fbox"]]));
				pfbox.appendChild(this.fboxes[fbpos]);
				if(fbpos) this.fboxes[fbpos].classList.add("wait");
			}
			if(man.related){
				var reluri = Mut.obj.get_oneuri(man.related);
				related = " (<a href=\"" + reluri + "\">" + 
			...
			img = "<img src=\"" + imgsrc + "\"/>",
			cls = link[0] ? "" : "class=\"direct \"",
			ancimg = link[2] === false ? img : "<a " + cls + "href=\"" + link.join("") + "\"" + tgattr + ">" + img + "</a>";
		...
		this.inc_ctrl = this.setup_inc_ctrl(pfbox);
		if(this.inc_ctrl){
			pnode.appendChild(this.inc_ctrl[0]);
			pnode.appendChild(pfbox);
			if(this.inc_ctrl[1]) pnode.appendChild(this.inc_ctrl[1]);
		}else pnode.appendChild(pfbox);
		if(!this.thumb_init_done){
			document.body.style.paddingRight = "5px";
			document.body.style.maxWidth = "none";
			this.thumb_init_done = true;
	...
	setup_inc_ctrl: function(pfbox, offset){
		if(typeof offset === "undefined") offset = 0;
		if(this.thumb_count <= this.thumb_per_div){
			var dispres = this.thumb_count + " result" + (this.thumb_count > 1 ? "s": "");
			return [Mut.dom.elt("span", dispres, [["class", "ctrl0"]])];
		}
		var that = this,
		ctrlparam = {
			left: {text: "≪ prev", dir: -1, inistate: "wait"},
			right: {text: "next ≫", dir: +1, inistate: "active"},
			unit: this.thumb_per_div,
			from: offset + 1,
			to: offset + this.thumb_per_div,
			all: offset + this.thumb_count
		},
		ctrls = this.create_inc_ctrl(ctrlparam),
		switcher = Mut.dom.elt("span", "show all", [["class", "switcher"]]);
		this.inc = {
			ctrl: ctrls,
			showpos: 0,
			maxpos: Math.floor((this.thumb_count - 1) / ctrlparam.unit),
			unit: ctrlparam.unit,
			all: ctrlparam.all,
			offset: 0
		};
		switcher.addEventListener("click", function(ev){
			if(pfbox.classList.contains("showall")){
				pfbox.classList.remove("showall");
				ev.target.innerText = "show all";
			}else{
				pfbox.classList.add("showall");
	...
	create_inc_ctrl: function(param){
		...
		for(var i=0; i<2; i++){
			var ctrl = {
				p: Mut.dom.elt("p"),
				status: Mut.dom.elt("span")
			};
			ctrl.p.className = "incCtrl ctrl" + i;
			ctrl.status.innerText = " " + param.from + " - " + param.to + " / " + param.all + " ";
			["left", "right"].forEach(function(key){
				ctrl[key] = Mut.dom.elt("span", param[key].text);
	...
	set_view: function(direction){
		if(direction < 0 && this.inc.showpos > 0){
			this.fboxes[this.inc.showpos--].classList.add("wait");
			this.fboxes[this.inc.showpos].classList.remove("wait");
			for(var i=0; i<2; i++){
		...
		+ (Math.min((this.inc.showpos+1) * this.inc.unit, this.thumb_count) + this.inc.offset)
		+ " / " + this.inc.all + " ";
	},
...
Miiif.searchs = {
	...
	setup: function(s){
		...
		var searchuri = s[Miiif.check_idattr(s, "search service")],
	...
	request: function(kwd){
		...
		if(Keyword === ""){
			...
			var oa = {};
			this.req = true;
			Muib.appb.set_msg("Asking to Search API ...", "show");
			Muib.appb.set_status("wait");
			OpenSeadragon.makeAjaxRequest({
				...
				"success": function(xhr) {
					...
					setup_anno(oa);
					present_anno("new");
					Muib.appb.set_msg("done");
				},
				"error": function(e){
					console.error(e);
					Muib.appb.set_msg("done with error");
				}
			});
		}
		return false;
		
		function present_anno(mode){
			var add_remove;
			if(mode === "restore"){
				that.showing = "normal";
		...
		function setup_tindex(mode){
			if(mode === "restore"){
				Mia.tindex.restore(that.saved, true);
			}else if(mode === "same"){
				Mia.tindex.restore(that.lastkw);
				that.prepare_reset(Mia.tindex.elt.querySelector("li.reset"));
			}else{
				if(!Miiif.searchs.last_kwd){
					that.saved.ti = Mia.tindex.elt.innerHTML;
					that.saved.tlist = Mia.tindex.list;
					that.saved.an = Mia.tindex.an;
				}
				Mia.tindex.an = {"label": " occur", "pfx": "✑ "};
				Mia.tindex.elt.innerHTML = "";
				Mia.tindex.list = [];
				gen_ul();
				Mia.tindex.update(null);
				that.lastkw.ti = Mia.tindex.elt.innerHTML;
				that.lastkw.tlist = Mia.tindex.list;
			}
		...
		function gen_ul(){
			...
			ullist.appendChild(Mut.dom.elt("li", 
				(Muib.state.missed_embed ? "(" + Muib.state.missed_embed + " unknown target)" : "(not found)")
			, [["class", "notfound"]]));
...
OSDV.prototype = {
	...
	set_osd_handlers: function(){
		...
		this.viewer.addHandler("open-failed", function(e){
			...
			Muib.appb.set_status("auto");
			var fail_msg = Mpj.test_support() || "Failed to open " + (
			...
			Muib.appb.set_msg(fail_msg, "error");
			Muib.state.is_error = true;
			...
			}else if(mmia.get_pos() === 0 && mmia.env.numPages > 1 && !location.hash){
				console.log("trying to next page (test mode)");
				this.fail_msg = fail_msg;
				mmia.osdv.goTo(1);
				mmia.tindex.update_urlhash(2);
			}else{
...
var Tindex = function(mia) {
	...
	this.labels = {"max": {len: 0, text: ""}, "avg": 0, "acc": 0, "numlabels": 0, "idxlabels": 0};
	this.numpfx = "#";
...
Tindex.prototype = {
	...
	init_vinfo: function(inipos, from_search){
		...
		sfx = " (" + Mut.html.abbr(this.an.pfx, "annotations", true) + " " + this.anno_disp();
		...
		function test_paged(current){
			...
			if(is_continuous) return current_btn("continuous");
			else if(current === "paged"){
				changeto = "indv";
		...
		function test_dir(current){
			...
			if(is_continuous) return current_btn(current, abbrs[current]);
			else if(current === "ltr"){
				changeto = "rtl";
				return switch_btn(changeto, method, abbrs[changeto])
				 + " "+ current_btn(current, abbrs[current]) ;
		...
		function changer(label){
			return "<span class=\"pseudolink\" onclick=\"Mia.tindex.change_view("
			+ (Miiif.vinfo.view[0] === "continuous" ? "'void'"  : "") + ");\">" + label +"</span>";
		}
	...
	restore: function(xsaved, restore_an){
		this.elt.innerHTML = xsaved.ti;
		this.list = xsaved.tlist;
		this.ppos = this.elt.querySelector("#ppos");
		if(restore_an) this.an = xsaved.an;
		this.elt.querySelectorAll("li[data-p]").forEach(function(li){
			this.set_li_hander(li);
		}, this);
		if(Muib.state.struct){
			this.elt.querySelectorAll("li.range span").forEach(function(toggler){
				toggler.onclick = function(ev){Mia.struct.toggole_or_link(this, ev);};;
			});
			var stab;
			if((stab = document.getElementById("strctab"))){
				Mia.struct.strctab.base = stab;
				var stabs = Mia.struct.strctab.tabs = stab.querySelectorAll("span");
				stabs.forEach(function(t, i){t.onclick = function(e){Mia.struct.reset(e);}; });
			}
		}
		var last_current = this.elt.querySelector("li.current");
		if(last_current) last_current.classList.remove("current");
		this.lup(this.elt.querySelector("li[data-i='" + Muib.state.pos + "']"));
		this.ppos_update(Muib.state.pos);
	},
	...
	lup: function(tgobj){
		this.clicktg = tgobj;
		if(Mav.type) this.lup_av(tgobj);
		else this.lup_img(tgobj);
		Muib.appb.msg.count = 0;
	},
...
Struct.prototype = {
	...
	setup: function(annot, mode){
		...
		if(Muib.state.struct === true) this.refresh();
		...
		var idxlabel_ratio;
		if(mode) this.show_type = ((mode === "bkmark" || mode === "ranges") ? this.SHOW_RANGE_ONLY : this.SHOW_FULL);
		else if(this.mia.opts.v.struct){
			if(this.mia.opts.v.struct === "bookmark") this.show_type = this.SHOW_RANGE_ONLY;
			if(this.mia.opts.v.struct === "full") this.show_type = this.SHOW_FULL;
			else if(this.mia.opts.v.struct.match(/^(semi|mid)$/)) this.label_len_type = this.mia.opts.v.struct;
		}else if(Miiif.curation) this.show_type = this.SHOW_FULL;
		else if((idxlabel_ratio = this.tindex.calc_numlabel_ratio("idx")) < 0.5){
			this.show_type = this.SHOW_FULL;
		...
		this.idattr = Miiif.check_idattr(this.data[0], "structures");
	...
	generate: function(){
		...
		this.data.forEach(function(r){
			...
			if(r[this.memberp]) this.proc_members(el, r[this.memberp], rsid, is_single, true);
		...
		else if(this.ullist.childNodes.length === 1){
	...
	proc_members: function(elm, members, rsid, is_single, from_top){
		var idattr = members[0][Miiif.a.id] ? Miiif.a.id : Miiif.a.failid;
		members.forEach(function(rm, i){
			if(rm[Miiif.a.vhint]==="no-nav") return;
			var membid = rm[idattr] || rsid + "-" + i;
			...
			if(this.is_leaf(rm)){
				...
				if(rm[this.memberp]) {
					...
					this.proc_members(elmm, rm[this.memberp], membid, this.test_single(rm), false);
				}else{
				}
			}
		}, this);
		if(!from_top) this.register(elm, rsid);
	},
	...
	prep_elt: function(label, is_opener, hint){
		...
		toggler.className = is_opener ? "opener" : "closer";
		toggler.onclick = function(ev){that.toggole_or_link(this, ev);};
		...
		if(ev.clientX - obj.getBoundingClientRect().left < 16){
			...
			if(obj.className.match(/opener/)) {
				while(opener){
					...
					if(opentg.firstChild.className === "range") opener = opentg.firstChild.firstChild;
					else opener = null;
				};
			}
;
	...
	set_strctab: function(){
		...
		this.strctab.tabs.forEach(function(t, i){
			if(i === this.show_type) t.className = "active";
			else t.className = "pseudolink";
		}, this);
	...
	reset: function(e){
		...
		if(!ullist){
			...
			if(e.target.innerText === this.no_struct_label) this.show_type = this.SHOW_NONE;
			else ullist.className = "struct";
			if(!ti.elt) {
...
Layers.prototype = {
	...
	setup: function(layer, is_choice, t2b, ini_opc){
		...
		if(!this.elt){
			...
			this.flipper.onclick = function(){that.flip();};
			...
			if(!that.myelt){
				...
				Object.keys(that.mia.cinfo).forEach(function(uri, i){
					...
					Mut.dom.append(fb, [
						...
						Mut.dom.elt("figcaption", Mut.html.trim_text(label, "min"))
					]);
					that.boxes.push(fb);
...
Canvas.prototype = {
	proc: function(tiles, cv, info){
		...
		if(this.is_subcanvas){
			...
			if(this.thumbcand) set_thumb(this, this.thumbcand);
			else if(cv.thumbnail) set_thumb(this, Mut.obj.get_oneuri(cv.thumbnail));
			if(this.iiif.vinfo.thumbnail && this.iiif.vinfo.thumbnail === Mut.obj.get_oneuri(cv.thumbnail)){
				console.log(this.cvuri, this.iiif.vinfo.thumbnail);
				Mia.opts.v.canvas = this.cvuri;
			}//else console.log(this.iiif.vinfo.thumbnail, cv.thumbnail);
	...
	proc_one_item: function(item, default_type){
		...
		body_arr.forEach(function(bodyr){
			...
			if(bdformat) bdtype = confirm_type_format(bdtype, bdformat);
			switch(bdtype){
			case "Image" :
			...
			case "Canvas":
				...
				break;
			case "Annotation" :
				if(bodyr.format.match(/^image/)){
					console.warn("wrong resource type", bdtype, "for", bodyr.format);
					this.image_and_tile(bodyr, trgt, item.stylesheet);
					break;
				}
			default :
				Miiif.set_error("Unknown canvas resource type: " + bdtype, this.info);
				bdtype = false;
			}
			if(bdtype) Miiif.last_valid_type = bdtype;
		}, this);
		function confirm_type_format(type, format){
			if(type !== "Image") return type;
			else if(typeof(format) !== "string"){
				console.warn("Format attribute value should be a string, while this is:", format);
				return type;
			}else if(format.match(/^image/)) return type;
			switch(format){
			...
			case "text/html":
				...
				break;
			default:
				if(format.match(/^video/)) type = "Video";
				else if(format.match(/^audio/)) type = "Audio";
			}
			console.warn("type and format mismatch: Image vs ", format, "--> type set to " + type);
			return type;
		}
	...
	set_choice: function(bodyr, trgt, itemid, styles){
		...
		case "Image" :
			...
			if(bodyr.items){
				...
				if(bodyr.default) loct = this.image_and_tile(bodyr.default, trgt, styles);
				else loct = this.calc_loct(trgt);
			}
			items.forEach(function(item){
		...
		default:
			...
			Mut.arr.add(this.annot, this.cvuri, {
				id: itemid,
				target: typeof(trgt) === "string" ? {id: trgt} : trgt, 
				bodyValue: Mut.str.lang_obj(items)
			});
	...
	guess_type: function(bodyr, trgt, styles){
		var id = Mut.obj.get_oneuri(bodyr); //typeof(r) === "string" ? r : r[this.a.id];
		if(!id && bodyr.source){
			bodyr = bodyr.source;
	...
	set_tile: function(bodyr, loct, styles){
		if(!bodyr) return  {"error": "no image resouce"};
		...
		if(rfull.service){
			...
			if(!uri){
				...
				if(uri) console.warn("no", this.a.id, "but", this.a.failid, "found for the service");
				else console.error(this.a.id, "for the service not found");
			...
			if(rsrvs.profile === "http://schemas.microsoft.com/deepzoom/2008"){
				...
				loct[3] = this.iiif.ssl.test(cleanid + "/info.json");
				this.cinfo.iiif_service = cleanid;
				if(rsrvs.tiles 
					&& rsrvs.protocol
				){
					tsource = {tileSource: rsrvs};
				}else if(this.mia.opts.v.noimgservice){
					tsource = preset_infojson(uri, this.cv.width, this.cv.height);
				}else tsource = loct[3];
				if(this.iiif.level.imgapi < 1) this.iiif.level = check_levels(rsrvs, this);
				this.thumbcand =
		...
		function check_levels(s, that){
			...
			else if(s.profile && s.profile.match(/level([\d\.]+)$/)) level.imgapi = Number(RegExp.$1);
	...
	set_canvas_meta: function(obj){
		var meta = "", mar = [],
		more = {"attribution": "Attribution", "seealso": "See also", "related": "Related", "rendering": "Rendering", "within": "Within", "partOf": "Part of"};
...
var Muib = {
	...
	appb: {
		...
		set_ogp: function(imgurl){
			var h = document.head;
			Mut.dom.append(h, Mut.dom.elt("meta", "", [["property", "og:title"], ["content", document.title]]));
			Mut.dom.append(h, Mut.dom.elt("meta", "", [["property", "og:type"], ["content", "website"]]));
			Mut.dom.append(h, Mut.dom.elt("meta", "", [["property", "og:image"], ["content", imgurl]]));
			Mut.dom.append(h, Mut.dom.elt("meta", "", [["property", "og:site_name"], ["content", "The Web KANZAKI"]]));
			Mut.dom.append(h, Mut.dom.elt("meta", "", [["property", "twitter:card"], ["content", "summarylargeimage"]]));
		},
		set_msg: function (msg, cls, wait, msgbox){
			...
			elt.innerHTML = msg.replace(/\.\.\.$/, "<img src=\""+ Mia.const.parts_path + "loading-c.gif\"/>");
			if(msg.substr(0, 4) === "done"){
				...
				else if(cls === "annot") this.set_status("wait");
	...
	meta: {
		...
		added: false,
		div: {
			meta: {id: "metap", label: "metadata", mdata: {}, count:0, show: false},
			link: {id: "linkp", label: "link properties", mdata: {}, count:0, show: false}
		},
		linkp : {seeAlso: "See also", homepage: "Homepage", related: "Related", rendering: "Rendering", within: "Within", partOf: "Part of"},
		temp: {meta: {}, link: {}}, 
		numprop: 0,
		add: function (def){
		...
		generate: function(def, pelt){
			...
			this.numprop = 0;
			for(var key in this.temp) this.temp[key] = {mdata: {}, count:0};
			...
			pelt.innerHTML = "<em>Document info</em> <span class=\"doctype\">" +
			"<a href=\"" + (Mia.opts.v.org_u || Mia.opts.v.u) + "\">" +
		...
		set_metalist: function(meta, tgobj, saved){
			var that = this;
			if(!tgobj) tgobj = {mdata:{}, count:0};
			if(meta){
				if(meta instanceof Array) meta.forEach(add_meta);
				else add_meta(meta);
			}
	...
	anno: {
		...
		jldpanel: {
			...
			showbtn: function (action){
				...
				if(action === "selector"){
					...
					if(!this.mode){
						if(Mia.anno_count > 1000){
							this.btnlabel += " (page)";
							Mia.elt.jldb.firstChild.data += " (page)";
							this.mode = "page";
						}else{
							this.mode = "full";
						}
			...
			toggle: function (o, id){
				...
				if(label.data.substr(0, 5) === "Close"){
					...
					Muib.appb.set_status("wait");
					var as_page_chbx = {"checked": this.mode === "full" ? false : true},
					ev = window.event;
					jldarea.value = "";
		...
		clip: {
			...
			init_elt: function(jldctrl){
				...
				this.btn.onclick = function(){Muib.anno.clip.toggle();};
			...
			gen_clip: function (parnt, allimg){
				...
				function as_css(an, uri){
					...
					if(Mia.cinfo[uri].imgurl){
						if(Miiif.level.imgapi === 0){
							dim = {"x": Mia.cinfo[uri].dim.x};
							uri = Mia.cinfo[uri].imgurl;
						}else{
							dim = {"x": 1000};
		...
		walk: {
			...
			init_elt: function(jldctrl){
				...
				this.btn.onclick = function(ev){Muib.anno.walk.go(1, ev);};
			...
			show: function(pos, delta=1, cleapos){
				...
				if(an.hasOwnProperty("rotation")) vport.setRotation(an.rotation);
				else if(vport.getRotation() !== 0) vport.setRotation(0);
			...
			debut: function(){
				...
				Mia.osdv.viewer.addHandler("animation-finish", function(){Muib.anno.walk.hilite();});
			},
			fin: function(){
				var viewer = Mia.osdv.viewer,
			...
			onkey: function(ev){
				...
				case "ArrowLeft":
					...
					break;
				case "Escape":
					if(Muib.anno.walk.is_auto) Muib.anno.walk.position = Manno.current.length;
					else Muib.anno.walk.fin();
					break;
				}
			},
		...
		abox: {
			...
			init_elt: function(jldctrl){
				...
				this.btn.onclick = function(){Muib.anno.abox.toggle();};
				this.btn.style.display = "none";
				Mut.dom.append(jldctrl, [" ", this.btn]);
			},
				...
				if(obj.nodeName.toLowerCase()==="a"){
					...
					if(dest && dest.charAt(0)==="#"){
						...
						if(dest.match(/^#p(\d+)/)){
							...
							setTimeout(function(){Muib.anno.popup.show_one(0, moved, anid);}, 300);
						}else if(dest === "#!prev"){
							moved = go(null, -1, ds, this);
							setTimeout(function(){Muib.anno.popup.show_one(-1, moved, anid);}, 300);
						}
		...
		load_script: function(src, use_libpath){
			...
			if(!script){
				script = Mut.dom.elt("script","",[
					...
					["id", (use_libpath ? src : Mut.uri.filename(src))]]);
...
var Mut = {
	...
	html: {
		...
		fold_text: function (text, type="long", fold=true){
			...
			if(notag.length > limit){
				...
				: "<span title=\"" + notag + "\">" + notag.substr(0, thld) + "...</span>";
			}else{
				res = text;
			}
			return res.replace(/\\n/g, "<br/>");
	...
	frag: {
		...
		parse_selector: function(tg){
			...
			svgfrag = "";
			if(seltype === "SvgSelector") svgfrag = sel.value;
			else if(seltype === "Choice" && sel.item && Miiif.get_type(sel.item) === "SvgSelector") svgfrag = sel.item.value;
			if(frag) return this.as_array(frag, base, svgfrag);
			else return [base, "", "", "", svgfrag];
		},
